...
@persistence.persistent_class
class CEDMPrecond(torch.nn.Module): 
    def __init__(self,
        img_res_h,                          # Image resolution h.
        img_res_w,                          # Image resolution w.
        img_input_channels,                 # Number of input channels.
        img_cond_channels,                  # Number of conditional color channels.
        label_dim  = 0,                     # Number of class labels, 0 = unconditional.
        use_fp16   = False,                 # Execute the underlying model at FP16 precision?
        sigma_min  = 0,                     # Minimum supported noise level.
        sigma_max  = float('inf'),          # Maximum supported noise level.
        ...
        **model_kwargs,                     # Keyword arguments for the underlying model.
        ...
        self.sigma_max          = sigma_max
        self.sigma_data         = sigma_data
        model_in_channels       = img_input_channels + img_cond_channels
    ...
    def forward(self, x, sigma, conditions=None, class_labels=None, force_fp32=False, **model_kwargs):
        ...
        F_x = self.model(aug.to(dtype), c_noise.flatten(), class_labels=class_labels, **model_kwargs)
        assert F_x.dtype == dtype
        D_x = c_skip * x + c_out * F_x.to(torch.float32)
        return D_x
