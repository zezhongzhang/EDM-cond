...
@click.option('--data',          help='Path to the dataset', metavar='ZIP|DIR',                     type=str, required=False)
...
def main(**kwargs):
    ...
    CACHE = True
    c = dnnlib.EasyDict()
    c.dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset_cond.CondDataset', 
                                       path=opts.data, 
                                       img_res_h=IMG_RES_H, img_res_w=IMG_RES_W, 
                                       img_input_channels=IMG_CHANNELS, img_cond_channels=COND_CHANNELS,
                                       xflip=XFLIP, yflip=YFLIP, cache=CACHE)
    ...
    try:
        ...
        dataset_name = 'TestDataset'  # Placeholder for dataset name
